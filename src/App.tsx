...
function Hero() {
  ...
  const defaultHeroImage = "https://storage.googleapis.com/aistudio-user-attachments/6f1118a8-b63e-4861-9f2d-8646b955c48b/nnimal_logo.png";
    ...
    <section id="community" className="w-full py-24 md:py-40 px-6 md:px-16 bg-[#f9f9f7]">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
          <h3 className="font-headline text-xl tracking-[0.1em] uppercase mb-2">Belum ada postingan komunitas</h3>
          <p className="text-[10px] font-body tracking-[0.15em] text-[#777777] uppercase">Nantikan update terbaru dari komunitas kami.</p>
        </div>
      ) : (
                ...
                {isEven ? (
                  <>
                    {item.imageUrl ? (
                      ...
                      <div className="aspect-[4/5] overflow-hidden bg-[#f4f4f2] flex items-center justify-center">
                        <span className="text-[10px] uppercase tracking-widest text-[#777777]">No Image</span>
                      </div>
                    )}
                  ...
                  <>
                    ...
                    {item.imageUrl ? (
                      ...
                      <div className="aspect-[4/5] overflow-hidden bg-[#f4f4f2] order-1 md:order-2 flex items-center justify-center">
                        <span className="text-[10px] uppercase tracking-widest text-[#777777]">No Image</span>
                      </div>
                    )}
                  </>
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubHero = onSnapshot(doc(db, 'settings', 'hero'), (docSnap) => {
      if (docSnap.exists()) {
        setHeroFormData({ imageUrl: docSnap.data().imageUrl || '' });
      }
    });
      ...
      <div className="flex-1 p-6 md:p-16 max-w-6xl mx-auto w-full">
        ...
        <div className="flex space-x-8 mb-12 border-b border-gray-200 overflow-x-auto hide-scrollbar">
          <button 
            ...
            className={`pb-4 text-[10px] uppercase tracking-widest transition-colors whitespace-nowrap ${activeTab === 'hero' ? 'border-b-2 border-[#1a1c1b] font-medium' : 'text-[#777777]'}`}
          >
            Hero Banner
          </button>
          <button 
        ...
        {activeTab === 'hero' ? (
          <div className="max-w-md">
            <h2 className="text-[12px] uppercase tracking-widest font-medium mb-8">Hero Banner Settings</h2>
